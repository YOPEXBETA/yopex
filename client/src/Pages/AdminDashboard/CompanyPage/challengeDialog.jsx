...
const ChallengeDialog = ({ open, handleClose, company }) => {
  ...
  const { data } = useQuery({
    queryKey: ["challenges", company._id],
    queryFn: async () => {
      const { data } = await axios.get(
        `http://localhost:8000/company/get/${company._id}`,
  ...
  const totalPages = Math.ceil(data?.challenges.length / cardsPerPage);
      ...
      <Stack m={"0px 200px"}>
        ...
        <Stack flexDirection={"column"} alignItems={"flex-end"}>
          {data?.challenges.length > 0 &&
            data.challenges.slice(startIndex, endIndex).map((card) => (
