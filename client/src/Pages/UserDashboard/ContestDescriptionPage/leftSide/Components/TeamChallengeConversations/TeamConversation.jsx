...
import {
    useCreateTeamChallengeMessage, useCreateTeamMessage,
    useGetTeamChallengeConversation,
    useGetTeamChallengeMessages, useGetTeamConversation, useGetTeamMessages
...
const TeamConversation = ({challenge, team}) => {
    const chatContainerRef = useRef(null);
    const [membersVisible, setMembersVisible] = useState(false);
    const [sidebarVisible, setSidebarVisible] = useState(false);
    const [showEmojiPicker, setShowEmojiPicker] = useState(false);
    const { data: teamConversation } = useGetTeamConversation(challenge?._id, team?.team?._id);
    console.log('teamConv', teamConversation)
    const [members, setMembers] = useState([]);
    const conversationId = teamConversation?._id;
    const { user } = useSelector((state) => state.auth);
    const { data: messages } = useGetTeamMessages(conversationId);
    const { mutate } = useCreateTeamMessage(conversationId);
    ...
    useEffect(() => {
        if (!socket) return;
        socket.on("getTeamMessage", (data) => {
            setArrivalMessage((prev) => [...prev, data]);
        });

        return () => {
            socket.off("getTeamMessage");
        };
    }, [socket]);

    useEffect(() => {
        if (teamConversation) {
            setMembers(teamConversation.members);
        }
    }, [teamConversation]);
    useEffect(() => {
            ...
            {sidebarVisible && (
                <div className="col-span-1 bg-white rounded-lg shadow-md ml-2 flex flex-col">
                    <div className="flex text-center pt-2 items-center flex-col">
                        <img
                            src={team?.team?.picturePath || AvatarProfile}
                            className="object-cover h-24 w-24 rounded-full shadow-md"
                            alt=""
                        />
                        <div className="font-semibold pt-4">Your Team Conversation</div>
                        <div className="py-2">Created {format(new Date(team.registrationDate), "dd MMM yyyy")}</div>
