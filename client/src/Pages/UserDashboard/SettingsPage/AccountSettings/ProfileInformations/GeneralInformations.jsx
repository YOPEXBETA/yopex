...
const GeneralInformations = ({ extra }) => {
  ...
  const { data: professions } = useOccupations("");
console.log('user', user)
  ...
  const defaultOccupation = formattedProfessions?.find(
      (profession) => profession.value === user?.occupation
  );
  ...
  const onSubmit = async (data) => {
    const { file, skills, occupation, ...values } = data;
    ...
    if (data.file.length > 0) {
      ...
      const occupationId = occupation ? occupation.value : null;
      return dispatch(
        edit({
          ...values,
    ...
    return dispatch(
      edit({
        ...
        occupation: occupation ? occupation.value : null,
      })
    );
  };
